...
//  SetGame
//
//  Created by Shmygovskii Ivan on 4/11/19.
...
@IBDesignable class SetCardButton: BorderButton {
    ...
    var setCard: Card? = Card(color: Card.CharacteristicVariant.third, symbol: Card.CharacteristicVariant.third, count: Card.CharacteristicVariant.third, filling: Card.CharacteristicVariant.third)
        { didSet{updateButton()}}
    ...
    private func setAttributedString(card: Card) -> NSAttributedString{
        ...
        let symbolsString = symbol.join(n: card.count.rawValue, with: separator)
        //------ attributes: color & fill-------
        let attributes:[NSAttributedString.Key : Any] = [
            .strokeColor: colors[card.color.idx],
            .strokeWidth: strokeWidths[card.filling.idx],
            .foregroundColor: colors[card.color.idx].withAlphaComponent(alphas[card.filling.idx])
        ]
